...
class ScansControllerTest < ActionController::TestCase
  ...
  test "should create scan" do
    assert_difference('Scan.count') do
      post :create, :scan => { :event_id => @scan.event_id, :scanned_at => @scan.scanned_at, :value => @scan.value }
    end
  ...
  test "should update scan" do
    put :update, :id => @scan, :scan => { :event_id => @scan.event_id, :scanned_at => @scan.scanned_at, :value => @scan.value }
    assert_redirected_to scan_path(assigns(:scan))
  end
